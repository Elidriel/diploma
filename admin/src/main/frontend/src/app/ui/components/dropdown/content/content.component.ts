...
export class TCDropdownContentComponent implements OnInit, DoCheck {
  ...
  @HostBinding("class.tc-dropdown-content-inside")
  @Input()
  appendToBody: boolean;
  @HostBinding("class.tc-dropdown-global-positioning")
  @Input()
  ...
  // close from Esc key
  @HostListener("window:keydown", ["$event"]) keyboardInput(
    event: KeyboardEvent
  ) {
  ...
  applyOffset(top: number, left: number): { top: number; left: number } {
    ...
    if (
      ...
      (offset instanceof Array && offset.length === 0) ||
